...
**Description:** Allows customers and trainers to mark their own attendance. **IMPORTANT: Customers and trainers can ONLY mark attendance for today's date. Any attempt to mark attendance for past or future dates should be rejected by the backend.**

**Authentication:** Required (Customer or Trainer role)

**Business Rules:**
- The `date` field must be today's date (YYYY-MM-DD format matching the current server date)
- If a date other than today is provided, the backend should return a `400 Bad Request` error
- Users can update their attendance for today if they've already marked it
- Only admins can mark attendance for past or future dates

**Request Body:**
```json
{
  "date": "2024-01-15",  // Must be today's date
  "status": "present",
...
- **CRITICAL: The date must be today's date. Backend must validate and reject any date that is not today.**
- If attendance already exists for today, it should be updated (or return error with option to update)
- `marked_by` should be `null` for self-marked attendance
- Date must be in YYYY-MM-DD format
- Status must be either 'present' or 'absent'

**Error Responses:**
- `400 Bad Request`: Invalid date format OR date is not today's date
...
7. **Date Validation:**
   ...
   - **For customers/trainers marking own attendance (`POST /attendance/mark`): Only today's date is allowed**
   - **For admin endpoints (`POST /attendance`, `POST /attendance/bulk`): Future dates and past dates are allowed**
   - Backend must validate the date based on the endpoint and user role
